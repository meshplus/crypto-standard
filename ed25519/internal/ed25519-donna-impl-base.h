...
DONNA_INLINE static void
ge25519_p1p1_to_full(ge25519 *r, const ge25519_p1p1 *p) { //CompletedGroupElement
	curve25519_mul(r->x, p->x, p->t);
	curve25519_mul(r->y, p->y, p->z);
	curve25519_mul(r->z, p->z, p->t);
...
ge25519_pack(unsigned char r[32], const ge25519 *p) {
	...
	curve25519_recip(zi, p->z);   //zi = 1/p.z
	curve25519_mul(tx, p->x, zi); //tx = p.x/p.z
	curve25519_mul(ty, p->y, zi); //ty = p.y/p.z
	curve25519_contract(r, ty);
	curve25519_contract(parity, tx);
	r[31] ^= ((parity[0] & 1) << 7);  //tx的最高位异或ty的最低位, r <= ty
}

static int
ge25519_unpack_vartime(ge25519 *r, const unsigned char p[32], char negative) {
	...
	//打印 num * den^3 * (num*den^7)^((p-5)/8) 一致
...
#define S1_TABLE_SIZE (1<<(S1_SWINDOWSIZE-2)) //8
#define S2_SWINDOWSIZE 7
#define S2_TABLE_SIZE (1<<(S2_SWINDOWSIZE-2)) //32
...
ge25519_double_scalarmult_vartime(ge25519 *r, const ge25519 *p1, const bignum256modm s1, const bignum256modm s2) {
	...
	ge25519_pniels pre1[S1_TABLE_SIZE];  //s1的预计算表
	ge25519 d1;
	ge25519_p1p1 t;
	int32_t i;

    //这两个函数将256bit转换为256字节
	contract256_slidingwindow_modm(slide1, s1, S1_SWINDOWSIZE); //窗口是5  table 8
	contract256_slidingwindow_modm(slide2, s2, S2_SWINDOWSIZE); //窗口是7  table 32

	ge25519_double(&d1, p1);
	ge25519_full_to_pniels(pre1, p1);
	for (i = 0; i < S1_TABLE_SIZE - 1; i++)
		ge25519_pnielsadd(&pre1[i+1], &d1, &pre1[i]);  //计算P, 2P ... 8P

	/* set neutral */
	memset(r, 0, sizeof(ge25519));  //结果保存在r
	r->y[0] = 1;
	r->z[0] = 1;

	i = 255;
	while ((i >= 0) && !(slide1[i] | slide2[i]))  //高位是0的不用处理
		i--;

	for (; i >= 0; i--) {
		ge25519_double_p1p1(&t, r);   //r = 2r  高位向低位处理
...
ge25519_scalarmult_base_niels(ge25519 *r,  const bignum256modm s) {
	...
	ge25519_scalarmult_base_choose_niels(&t, ge25519_niels_base_multiples, 0, b[1]);
	...
	r->z[0] = 2;
	for (i = 3; i < 64; i += 2) {
		ge25519_scalarmult_base_choose_niels(&t, ge25519_niels_base_multiples, i / 2, b[i]);
	...
	ge25519_double_partial(r, r);
	ge25519_double(r, r);
	ge25519_scalarmult_base_choose_niels(&t, ge25519_niels_base_multiples, 0, b[0]);
