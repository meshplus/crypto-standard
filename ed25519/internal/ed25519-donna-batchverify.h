...
ge25519_multi_scalarmult_vartime(ge25519 *r, batch_heap *heap, size_t count) {
    ...
    size_t limbsize = bignum256modm_limb_size - 1;  //always 4
    ...
    heap_build(heap, ((count + 1) / 2) | 1); //第二个参数必须大于等于3
    for (;;) {
...
//return 0 or 1 or 2
//int
//ED25519_FN(ed25519_sign_open_batch)(const unsigned char **m, size_t *mlen, const unsigned char **pk,
...
//        for (i = 0; i < batchsize; i++)
//            if (!ge25519_unpack_vartime(&batch.points[i + 1], pk[i], 1))
//                goto fallback;
//        for (i = 0; i < batchsize; i++)
//            if (!ge25519_unpack_vartime(&batch.points[batchsize + i + 1], RS[i], 1))
//                goto fallback;
//
...
//
//    for (i = 0; i < num; i++) {
//        int valid = ED25519_FN(ed25519_sign_open)(m[i], mlen[i], pk[i], RS[i]) ? 1 : 0;
//        return valid;
//    }
//
//    return 0;
...
//return 0 or 1 or 2
int
ED25519_FN(ed25519_sign_open_batch_pre)(batch_heap *batch,
                                        const unsigned char **m, uint32_t *mlen, const unsigned char **pk,
                                        const unsigned char **RS, size_t batchsize) {
    bignum256modm *r_scalars = &batch->scalars[batchsize + 1];
    size_t i;
    unsigned char hram[64];
    bignum256modm sum;
    /* generate r (scalars[batchsize+1]..scalars[2*batchsize] */
    ...
    r_scalars = &batch->scalars[batchsize + 1];
    for (i = 0; i < batchsize; i++) {
        ed25519_hram(hram, RS[i], pk[i], m[i], (size_t)mlen[i]);
        expand256_modm(batch->scalars[i + 1], hram, 64);
        r_scalars[i][0] = batch->scalars[i + 1][0];
        r_scalars[i][1] = batch->scalars[i + 1][1];
        mul256_modm(batch->scalars[i + 1], batch->scalars[i + 1], r_scalars[i]);
    }

    /* compute scalars[0] = ((r1s1 + r2s2 + ...)) */
    expand256_modm(batch->scalars[0], RS[0] + 32, 32);
    mul256_modm(batch->scalars[0], batch->scalars[0], r_scalars[0]);
    for (i = 1; i < batchsize; i++) {
        expand256_modm(sum, RS[i] + 32, 32);
        mul256_modm(sum, sum, r_scalars[i]);
        add256_modm(batch->scalars[0], batch->scalars[0], sum);
    }

    /* compute points */
    batch->points[0] = ge25519_basepoint;
    for (i = 0; i < batchsize; i++)
        if (!ge25519_unpack_vartime(&batch->points[i + 1], pk[i], 1)) return 1;
    for (i = 0; i < batchsize; i++)
        if (!ge25519_unpack_vartime(&batch->points[batchsize + i + 1], RS[i], 1)) return 1;
    return 0;
}

//this function only for test
int
ED25519_FN(ed25519_test_heap)(batch_heap *batch, size_t batchsize) {
    int i = 0;
    printf("%d\n", batch->size);
    printf("1:%d,10:%d,19:%d,40:%d,heap_batch_size-1:%d\n", batch->heap[1],batch->heap[10],batch->heap[19],batch->heap[40],batch->heap[heap_batch_size-1]);
    for(i =0;i <heap_batch_size;i++){
        batch->heap[i] = 233;
    }
}

int
ED25519_FN(ed25519_sign_open_batch_end)(batch_heap *batch, size_t batchsize) {
    ge25519 ALIGN(8) p;
    ge25519_multi_scalarmult_vartime(&p, batch, (batchsize * 2) + 1);
