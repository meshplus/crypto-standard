...
test_batch_instance(batch_test type, uint64_t *ticks) {
	...
	/* batch verify */
	t = get_ticks();
	ret = ed25519_sign_open_batch(message_pointers, message_lengths, pk_pointers, sig_pointers, test_batch_count);
	*ticks = get_ticks() - t;
...
test_main(void) {
	...
	for (i = 0; i < 1024; i++) {
		...
		edassert(ed25519_sign_open((unsigned char *)dataset[i].m, i, pk, sig), i, "failed to open message 1");
	...
	for (i = 0; i < 1024; i++) curved25519_scalarmult_basepoint(csk[(i & 1) ^ 1], csk[i & 1]);
	...
	for (i = 0; i < 2048; i++) {
		...
		edassert(res, 0, "failed to open message 21");
